...
export default function ProductDetail() {
  ...
  async function handleWishlist() {
    ...
    try {
      ...
      setMessage({ text: `✓ Added "${name}" to your wishlist!`, type: "ok" });
    } catch (err) {
      setMessage({ text: err.response?.data?.message || "Couldn't save to wishlist.", type: "err" });
    } finally {
      setTimeout(() => setMessage({ text: "", type: "ok" }), 2500);
    }
  }
            ...
            {inStock ? (
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="border border-[var(--color-line)] p-2.5 rounded hover:border-[var(--color-circuit)] text-[var(--color-ink-soft)] hover:text-[var(--color-circuit)] transition-colors flex items-center justify-center"
                >
