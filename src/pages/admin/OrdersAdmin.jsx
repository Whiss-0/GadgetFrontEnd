          ...
          return (
            ...
            >
              <span className="font-[var(--font-mono)] text-sm whitespace-nowrap">
                PO-{String(id ?? o.orderId ?? o.OrderId ?? 0).padStart(5, "0")}
              </span>
              <span className="text-sm text-[var(--color-dark-ink)]/70">
                user #{o.user_id}
              </span>
              <span className="text-xs text-[var(--color-dark-ink)]/50 hidden sm:inline">
                {o.order_date ?? o.orderDate ?? o.OrderDate ? new Date(o.order_date ?? o.orderDate ?? o.OrderDate).toLocaleDateString() : ""}
              </span>
              <select
                value={o.status ?? "Pending"}
                disabled={updatingId === id}
                onChange={(e) => handleStatusChange(id, e.target.value)}
                className="admin-input-premium border border-[var(--color-dark-line)] rounded px-3 py-1.5 text-xs text-[var(--color-circuit)] outline-none cursor-pointer"
              >
